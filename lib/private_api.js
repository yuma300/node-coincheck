...
var createPostOption = function(url, api_key, secret_key, user_agent, nonce, params){
    var post = createPostParam([{nonce:nonce}, params]);
    return {
        url: url,
        method: 'POST',
        form: post,
...
var createPrivateApi = module.exports = function(api_key, secret_key, user_agent, nonce_func){
    ...
    var query = function(method, params){
        return lp.req(createPostOption(url() + '/' + method, api_key, secret_key, user_agent, nonce_func(), params)).
        then(JSON.parse).then(function(v){            
            console.log("yw_debug return request \n")
